...
public static class Runtime 
{
    ...
    public static List<UIText> TextEntities = new();
	...
	public static void Initialize(Type window = null)
	{
	...
	private static void HandleWindowTick(Type window)
	{
        ...
        RenderHandler renderer = null;
        var rendererEnabled = window.GetField("Renderer", BindingFlags.Public | BindingFlags.Static);
        if (rendererEnabled != null)
            renderer = (RenderHandler)rendererEnabled.GetValue(null);
        ...
        while (IsRunning)
		{
            ...
            foreach (UIText text in TextEntities)
            {
                if (text != null)
                    text.Tick();
                    text.Update();
